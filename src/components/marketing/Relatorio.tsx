    ...
    >
      ...
      <dl className="grid border-t border-ink-200 sm:grid-cols-2">
        {features.map((key, index) => (
          <div
            key={key}
            className="grid grid-cols-[2rem_1fr] gap-x-3 border-b border-ink-200 py-5 sm:odd:border-r sm:odd:pr-8 sm:even:pl-8"
          >
            ...
            <li key={row} className="border-b border-ink-200 py-6">
              ...
              <div className="mt-3 grid gap-x-10 gap-y-3 sm:grid-cols-2">
                <p className="text-sm text-ink-400">
                  <span className="editorial-label mr-2 text-ink-400">{t('compare_before')}</span>
